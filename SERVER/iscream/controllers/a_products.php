...
class Products extends CI_Controller
{
	...
	private function makeTopMovie($toplist)
	{
		...
		foreach ($toplist as $film)
		{
			...
			$list[$i]['runtime'] 						= date('j\h i\m\i\n', $film->runtime);
			$list[$i]['runtime_timestamp'] 				= $film->runtime;
			images_helper::save($film->poster->href, 'img/', $film->code);
			$list[$i]['poster'] 						= $film->poster->href;
			$list[$i]['poster']							= str_replace('/pictures/', '/r_x_190/pictures/', $list[$i]['poster']);
			$list[$i]['release_date'] 					= date('Y', strtotime($film->release->releaseDate));
			$list[$i]['release_timestamp'] 				= strtotime($film->release->releaseDate);
			$list[$i]['rating'] 						= (isset($film->statistics->pressRating) && isset($film->statistics->userRating)) ? ($film->statistics->pressRating + $film->statistics->userRating) / 2 : 0 ;
